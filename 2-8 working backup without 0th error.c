...
regionAttrs.regionBase = 0x33;
    ...
    while (round < (readRound+1))
    {
        ...
        status = NVS_write(nvsHandle, regionAttrs.regionBase, (void *) buffer, sizeof(buffer), NVS_WRITE_POST_VERIFY);
        Display_printf(displayHandle, 0, 0, "status >>>>>>>>>>>>>>>>> %d",  status);

//        if(status== (-7))
//    {regionAttrs.regionBase = (MAXBUFLEN + 1)+ regionAttrs.regionBase;
//        Display_printf(displayHandle, 0, 0, "re-writing>>>>> ");
//        Display_printf(displayHandle, 0, 0, "Writing to regionBase: 0x%x", regionAttrs.regionBase);
//        //NVS_write(nvsHandle, regionAttrs.regionBase, (void *) buffer, sizeof(buffer), NVS_WRITE_POST_VERIFY);
//        status = NVS_write(nvsHandle, regionAttrs.regionBase, (void *) buffer, sizeof(buffer), NVS_WRITE_POST_VERIFY);
//        Display_printf(displayHandle, 0, 0, " new status>>> %d",  status);
//
//    }
        ...
        Display_printf(displayHandle, 0, 0, "reading from regionBase: 0x%x", regionAttrs.regionBase);
        NVS_read(nvsHandle, regionAttrs.regionBase, (void *) rbuffer, sizeof(buffer));
 ...
 sleep(1);
    ...
    if(totalRead == fileSize)
    {
     ...
     regionAttrs.regionBase = 0x33;
        ...
        NVS_read(nvsHandle, regionAttrs.regionBase, rbuffer, sizeof(buffer));
        Display_printf(displayHandle, 0, 0, "Data at regionBase: 0x%x", regionAttrs.regionBase);
        Display_printf(displayHandle, 0, 0, "%s \n", rbuffer);
        regionAttrs.regionBase = (MAXBUFLEN + 1) + regionAttrs.regionBase;
        memset(rbuffer, 0, (MAXBUFLEN));
        roundn++;
